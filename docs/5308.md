TITLE:
[CLI]: Multi-node training with Horovod fails to start 
...
While a single-node, multi-gpu training works as expected when wandb is used within a PyTorch training code with Horovod, training fails to start when I use > 1 node.
...
parser.add_argument('--epochs', type=int, default=32000, metavar='N',
...
wandb: Run data is saved locally in /lus/grand/projects/RNAtoImage/src/test_horovod/sdl_workshop/distributedDeepLearning/Horovod/wandb/run-20230408_071302-2cm2rxkm
wandb: Run `wandb offline` to turn off syncing.
wandb: Syncing run dandy-shadow-10
wandb: ⭐️ View project at https://wandb.ai/tnnandi/mnist_hvd
wandb: 🚀 View run at https://wandb.ai/tnnandi/mnist_hvd/runs/2cm2rxkm
(hangs here for multi-node jobs)
```
...
Versions of relevant libraries: horovod 0.25.0,  
